...
"""
abstract type InwardsDispersal{R,W,N} <: NeighborhoodRule{R,W,N} end
...
$(FIELDDOCTABLE)
"""
@Probabilistic @Kernel struct InwardsBinaryDispersal{R,W,N} <: InwardsDispersal{R,W,N} end
...
$(FIELDDOCTABLE)
"""
@Kernel struct InwardsPopulationDispersal{R,W,N} <: InwardsDispersal{R,W,N} end

@inline applyrule(rule::InwardsPopulationDispersal, data, state::AbstractFloat, 
                  index, hoodbuffer) = 
    applykernel(neighborhood(rule), hoodbuffer)

@Layers @Kernel struct SwitchedInwardsPopulationDispersal{R,W,N,Th} <: InwardsDispersal{R,W,N} 
    threshold::Th
end
...
$(FIELDDOCTABLE)
"""
@Kernel struct PoissonInwardsPopulationDispersal{R,W,N} <: InwardsDispersal{R,W,N} end
